...
void luaD_callint(lua_State* L, StkId func, int nresults, bool preparereentry)
{
    ...
    if (fromyieldableccall)
    {
        ...
        // on yield, we have to set the CallInfo top of the C function including aa for expected results, to restore later
        if (yielded)
        {
...
int lua_yield(lua_State* L, int nresults)
{
    ...
    L->base = L->top - nresults; // protect stack aa below
    L->status = LUA_YIELD;
    return -1;
}
