...
struct Udata
{
    ...
    uint8_t tag; // 3
    int len; // 4
    udata_meta<struct LuaTable*> metatable; // 8
    char data[1]; // 16
};
...
{
    CommonHeader; // 0
    uint8_t maxstacksize; // 3
    uint8_t numparams; // 4
    uint8_t is_vararg; // 5
    uint8_t flags; // 6
    uint8_t nups; // 7
    proto_abslineinfo<int*> abslineinfo; // 8
    proto_debuginsn<uint8_t*> debuginsn; // 16
    proto_source<TString*> source; // 24
    Proto** p; // 32
    proto_typeinfo<uint8_t*> typeinfo; // 40
    proto_lineinfo<uint8_t*> lineinfo; // 48
    proto_userdata<void*> userdata; // 56
    GCObject* gclist; // 64
    proto_locvars<struct LocVar*> locvars; // 72
    void* execdata; // 80
    uintptr_t exectarget; // 88
    proto_debugname<TString*> debugname; // 96
    proto_upvalues<TString**> upvalues; // 104
    TValue* k; // 112
    Instruction* code; // 120
    const Instruction* codeentry; // 128
    int sizecode; // 136
    int sizetypeinfo; // 140
    int sizelineinfo; // 144
    int sizelocvars; // 148
    int linedefined; // 152
    int bytecodeid; // 156
    int sizeupvalues; // 160
    int linegaplog2; // 164
    int sizep; // 168
    int sizek; // 172
    FeedbackVectorSlot* feedbackvec; // 176
    uint32_t feedbackvecsize; // 184
    uint32_t funid; // 188
    Proto* optimized; // 192
    Proto* deoptimized; // 200
    uint64_t cost; // 208
} Proto;
// clang-format on
...
struct Closure
{
    ...
    union
    {
        ...
        {
            lua_CFunction f; // 24
            TString* debugname; // 32
            closure_debugname<const char*> debugname_DEPRECATED; // 40
            closure_cont<lua_Continuation> cont; // 48
            TValue upvals[1]; // 56
        } c;
        struct
        {
